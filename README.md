...
- **發布可見度走設定**:`pmb auto` 日更以 `YOUTUBE_PRIVACY`(預設 `public`)直接發公開;`pmb publish` / `pmb run` 仍預設 dry-run、`--approve` 才上傳。
...
| `pmb publish [--date] [--approve]` | Phase 5 | 發布(**預設 dry-run**;`--approve` 才上傳 YouTube,可見度依 `YOUTUBE_PRIVACY`) |
| `pmb run [--date] [--dry-run] [--approve]` | — | 全流程 fetch→research→assemble→gate(`--approve` 才上傳,可見度依 `YOUTUBE_PRIVACY`) |
| `pmb today [--date] [--no-upload]` | — | 一鍵完成今日**本機**作業:合成→發佈(前置缺則中止);供控台一鍵 / cron |
| `pmb auto [--wait-minutes N] [--research-local] [--no-pull] [--no-upload]` | — | **每日全自動**:等雲端研究(含 `claude/*` 分支退路)→等不到改本機研究→合成→上傳(預設 public)→通知;冪等、休市 skip |
...
>
> **dry-run 的意義**:正常營運**唯一的 gate 是「上不上傳」**(`publish` / `run` 加 `--approve` 才上傳;可見度由 `YOUTUBE_PRIVACY` 決定,日更預設 `public`)。取數/合成都是免費、直接跑真的;`research`/`assemble` 的 `--dry-run`(範例/靜音)只給自動化測試用。
...
正式日更為**全本機 local-first**(見下方 SOP)。若額外想在 claude.ai/code 掛雲端 routine
當加速器/備援(雲端先做完研究推上來,本機直接沿用、省一次本機研究 + 訂閱用量),設定見
[`prompts/cloud_routine.md`](prompts/cloud_routine.md),並把 `.env` 的 `RESEARCH_SOURCE`
設為 `cloud`。**雲端永遠不合成、不發布。**
...
**每個交易日 — 全自動模式(預設:全本機 local-first,人工只剩 1 步)**

一次性 `pmb autopilot install --time 19:30` 後,每個平日傍晚**這台 Mac 自己完成全部**:

1. **💻 `pmb auto`**(launchd 觸發):`git pull` 同步 → `pmb fetch` 取真實快照 → **headless Claude Code 研究**(`claude -p`,本機登入免 API key、有 web search;產物過 schema 驗證、失敗自動重試)→ commit `research: <date> 盤前研究產出(本機)` + push main → 合成(深色圖表 + 卡拉OK字幕 + BGM 母帶)→ 上傳 YouTube(**預設 public**,API 自動帶:合成內容揭露、播放清單、語言、非兒童)→ 桌面通知附 Studio 連結。
2. **🧑 你**:點通知進 YouTube Studio 看片。影片已是公開狀態,有問題才動手撤下。

(休市自動 skip;當天已上傳/已研究過不會重跑(冪等);任一步失敗桌面通知。報告仍為人工貼上。研究吃本機 Claude 訂閱用量,一天一次。)

**研究來源設定**(`.env` 的 `RESEARCH_SOURCE`):
- `local`(預設):不等雲端,直接本機研究。
- `cloud`:先等雲端 routine push main(含 `claude/*` 分支自動併回),逾時依 `LOCAL_RESEARCH_FALLBACK` 決定是否本機補跑——想沿用雲端 routine 當加速器時用這個。
...
4. **🧑 審影片 + 💻 發布**:控台內嵌看片;`pmb auto` 已自動發公開,控台的手動上傳按鈕沿用同一組設定。
...
- [x] Phase 5 — 發布(`pmb auto` 自動公開;`publish`/`run` 預設 dry-run + orchestrator)

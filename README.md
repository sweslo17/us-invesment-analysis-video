...
1. **💻 `pmb auto`**(launchd 觸發):`git pull` 同步 → `pmb fetch` 取真實快照 → **headless Claude Code 研究**(`claude -p`,本機登入免 API key、有 web search;產物過 schema 驗證、失敗自動重試)→ commit `research: <date> 盤前研究產出(本機)` + push main → 合成(深色圖表 + 卡拉OK字幕 + BGM 母帶)→ 上傳 YouTube(**private**,API 自動帶:合成內容揭露、播放清單、語言、非兒童)→ 桌面通知附 Studio 連結。
2. **🧑 你**:點通知進 YouTube Studio 看片 → 改『**公開**』。完成。

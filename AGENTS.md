# AGENTS.md — PMB(Pre-Market Macro Brief)專案指令

> 這是給 Codex 的專案指令檔。每個 session 開始請先讀本檔與 `premarket-macro-brief-system-plan.md`(本 repo 內,**正式規格,細節以它為準**)。本檔給的是操作方式與鎖定的決策。
...
**開發/執行分工(重要):**
- **本檔供 Codex 互動式開發**這個 repo(寫 code、測試、跑 dry-run)。
- **日更研究已部署為 Claude Code 雲端 routine**(claude.ai/code/routines,平日 19:30 台北,
  push 研究產物到 main;設定見 `prompts/cloud_routine.md`),電腦關機也跑。**Codex 不負責日更研究。**
- **確定性 pipeline**(圖表渲染、TTS、合成、上傳)是普通腳本,由本機 `pmb auto`(launchd)在
  routine 產出 artifacts 後執行,**不放進 agentic session**。
...
- 研究 LLM:Claude Code 雲端 routine 本身(同一 agent 做研究+寫作,見 `prompts/cloud_routine.md`);本機開發/測試時用 Anthropic API 跑同一份 prompt。
...
**Phase 6 — 部署成雲端 routine + 文件**(✅ 已完成:研究 routine 跑在 claude.ai/code)
- 把 `prompts/daily_research.md` 整理成可貼進雲端 routine 的版本;寫 README 說明如何註冊 routine(盤前排程)、如何接 orchestrator、如何放行發布。

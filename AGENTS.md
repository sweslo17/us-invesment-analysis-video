...
- **日更研究在本機自動跑**:launchd 觸發 `pmb auto` → headless Claude Code(`claude -p`)
  跑研究 → 驗證 → commit+push main → 接著合成/上傳。**Codex 不負責日更研究。**
- **確定性 pipeline**(圖表渲染、TTS、合成、上傳)是普通腳本,同一次 `pmb auto` 接續執行,
  **不放進 agentic session**。
...
- 研究 LLM:本機 headless Claude Code(`claude -p`,見 `pmb/research/local_runner.py`);開發/測試可用 Anthropic API 跑同一份 prompt(選配)。
